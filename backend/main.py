...
app = FastAPI(title="AI Telecom Assistant API", version="2.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
@app.get("/")
def root():
    return {"service": "AI Telecom Assistant API", "model": MODEL, "status": "online"}

@app.get("/health")
def health():
    return {"status": "ok", "llm_configured": client is not None, "model": MODEL}
